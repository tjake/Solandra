...
public class SolandraIndexWriter extends UpdateHandler {
		...
		try {
			...
			flushMonitor.execute(new Runnable() {
					...
					while (true) {
						try {
							...
							if (core != null) {
								...
								if (lastFlush == null
										|| lastFlush <= (System
												.currentTimeMillis() - CassandraUtils.cacheInvalidationInterval)) {
									flush(core);
									lastCoreFlush.put(core,
								...
								for (Map.Entry<String, AtomicInteger> entry : bufferedWrites
										.entrySet()) {
				...
				private void flush(String core) {
					...
					ByteBuffer cacheKey = CassandraUtils.hashKeyBytes(
							(core).getBytes(), CassandraUtils.delimeterBytes,
							"cache".getBytes());

					RowMutation rm = new RowMutation(CassandraUtils.keySpace,
							cacheKey);
					rm.add(new QueryPath(CassandraUtils.schemaInfoColumnFamily,
							CassandraUtils.cachedColBytes,
							CassandraUtils.cachedColBytes),
							FBUtilities.EMPTY_BYTE_BUFFER, System.nanoTime());
	...
	public int addDoc(AddUpdateCommand cmd) throws IOException {
		...
		try {
			...
			String batchMode = SolandraCoreContainer.activeRequest.get()
					.getParameter("batch");

			if (!coreInfo.bulk && !cmd.allowDups
					&& (batchMode == null || !batchMode.equals("true")))
				docId = IndexManagerService.instance.getId(coreInfo.indexName,
						key);
			...
			if (docId != null) {
				...
				docId = IndexManagerService.instance.getNextId(
						coreInfo.indexName, key, rms);
			...
			if (isUpdate)
				writer.updateDocument(indexName, idTerm,
						cmd.getLuceneDocument(schema), schema.getAnalyzer(),
						shardedId, false);
			else
				writer.addDocument(indexName, cmd.getLuceneDocument(schema),
						schema.getAnalyzer(), shardedId, false, rms);
	...
	public void delete(DeleteUpdateCommand cmd) throws IOException {
		...
		if (!cmd.fromPending && !cmd.fromCommitted) {
			...
			throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
					"meaningless command: " + cmd);
		}
		if (!cmd.fromPending || !cmd.fromCommitted) {
			numErrors.incrementAndGet();
			numErrorsCumulative.incrementAndGet();
			throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
					"operation not supported" + cmd);
		}
		...
		ByteBuffer keyKey = CassandraUtils.hashKeyBytes(
				(indexName + "~" + term.text()).getBytes(),
				CassandraUtils.delimeterBytes, "keys".getBytes());
		ByteBuffer keyCol = ByteBuffer.wrap(term.text().getBytes());

		List<Row> rows = CassandraUtils.robustRead(keyKey, new QueryPath(
				CassandraUtils.schemaInfoColumnFamily), Arrays.asList(keyCol),
		...
		if (rows.size() == 1) {
			...
			if (row.cf != null) {
				...
				if (col != null) {
					ByteBuffer idCol = col.getSubColumns().iterator().next()
							.name();
					...
					ByteBuffer sidName = ByteBuffer.wrap(String.valueOf(sid)
							.getBytes());
					...
					RowMutation rm = new RowMutation(CassandraUtils.keySpace,
							keyKey);
					rm.delete(new QueryPath(
							CassandraUtils.schemaInfoColumnFamily, keyCol),
							System.nanoTime());

					// Delete docId so it can be reused
					// TODO: update shard info with this docid
					ByteBuffer idKey = CassandraUtils.hashKeyBytes(
							subIndex.getBytes(), CassandraUtils.delimeterBytes,
							"ids".getBytes());
					RowMutation rm2 = new RowMutation(CassandraUtils.keySpace,
							idKey);
					rm2.delete(new QueryPath(
							CassandraUtils.schemaInfoColumnFamily, sidName),
							System.nanoTime());
