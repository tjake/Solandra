...
public class CassandraIndexManager
{
    ...
    private IdInfo nextReservedId(String indexName, NodeInfo[] shards, String myToken)
    {
        ...
        for (NodeInfo node : shards)
        {
            ...
            //logger.info(myToken+ "  startingOffset = "+startingOffset+ ", nextOffset = "+nextOffset);
            ...
            if (winningToken != null && ByteBufferUtil.string(winningToken).equals(myToken))
            {
                ...
                logger.info("Reserved "+numReserved+" ids for "+myToken+" shard "+node.shard+" from slot "+getRandomSequenceOffset(nextOffset));
            }
            else
            {
            ...
            {
                //logger.info("someone changed from under us");
                return null;
            }
        }
    ...
    private NodeInfo[] pickAShard(ShardInfo shards) throws IOException
    {
        ...
        synchronized (shards)
        {
            ...
            for (Map.Entry<Integer, NodeInfo> shard : shards.shards.entrySet())
            {
                ...
                // can we still use this shard (other nodes havent gobbled up the ids)?
