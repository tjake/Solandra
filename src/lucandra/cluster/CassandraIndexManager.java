...
public class CassandraIndexManager extends AbstractIndexManager
{
    ...
    private final ConcurrentMap<String, List<IdInfo>>     indexReserves = new MapMaker().expiration(expirationTime, TimeUnit.SECONDS).makeMap();
    private final ConcurrentMap<String, ShardInfo>        indexShards   = new MapMaker().expiration(expirationTime, TimeUnit.SECONDS).makeMap();
    ...
    public CassandraIndexManager(int shardsAtOnce, double collisionThreshold)
    {
        ...
        Random r = new Random(getNodeSeed(StorageService.instance.getToken()));
    ...
    public long internalIncrement(String indexName, String key)
    {
        ...
        rm.add(new QueryPath(CassandraUtils.schemaInfoColumnFamily, idCol, ByteBuffer.wrap(myToken.getBytes())), keyCol , System.currentTimeMillis());
        ...
        rm2.add(new QueryPath(CassandraUtils.schemaInfoColumnFamily, keyCol, idVal), FBUtilities.EMPTY_BYTE_BUFFER , System.currentTimeMillis());

        
        //Update last offset info for this shard
        RowMutation rm3 = updateNodeOffset(indexName, StorageService.instance.getToken(), idInfo.node, idInfo.offset);
    ...
    private List<IdInfo> reserveIds(String indexName, NodeInfo[] shards, String myToken)
    {
        ...
        for(NodeInfo node : shards){
            ...
            for (int i = offset + 1; i < (offset + reserveSlabSize + 1); i++)
            {
                ...
                rm.add(new QueryPath(CassandraUtils.schemaInfoColumnFamily, id, ByteBuffer.wrap(myToken.getBytes())), off , 0, expirationTime);
            ...
            List<Row> rows = CassandraUtils.robustRead(key, new QueryPath(CassandraUtils.schemaInfoColumnFamily), ids, ConsistencyLevel.QUORUM);
    ...
    {
        List<IdInfo> rsvpd = new LinkedList<IdInfo>();
    ...
    private NodeInfo addNewShard(String indexName)
    {
        ...
        RowMutation rm = updateNodeOffset(indexName, StorageService.instance.getToken(), nodes, 0); //offset 0
    ...
    {        
        ...
        rm.add(
                ...
                ByteBuffer.wrap(String.valueOf(offset).getBytes()), 0);
