...
public class IndexReader extends org.apache.lucene.index.IndexReader {
	...
	static {
		...
		try {
			new IndexWriter(mockDirectory, new SimpleAnalyzer(), true,
	...
	public Document document(int docNum, FieldSelector selector)
			throws CorruptIndexException, IOException {
		...
		System.arraycopy(docId.array(), docId.position() + docId.arrayOffset(),
				docIdBytes, 0, docIdBytes.length);

		keyMap.put(CassandraUtils.hashKeyBytes(indexName,
				CassandraUtils.delimeterBytes, docIdBytes), docId);
		...
		if (selector != null && selector instanceof SolandraFieldSelector) {
			...
			List<Integer> otherDocIds = ((SolandraFieldSelector) selector)
					.getOtherDocsToCache();
			fieldNames = ((SolandraFieldSelector) selector).getFieldNames();

			logger.debug("Going to bulk load " + otherDocIds.size()
					+ " documents");
			...
			for (Integer otherDocNum : otherDocIds) {
				...
				System.arraycopy(docKey.array(),
						docKey.position() + docKey.arrayOffset(), docKeyBytes,
						0, docKeyBytes.length);

				if (docKey == null)
					continue;

				keyMap.put(CassandraUtils.hashKeyBytes(indexName,
						CassandraUtils.delimeterBytes, docKeyBytes), docKey);
			}
		}
		...
		if (fieldNames == null || fieldNames.size() == 0) {
			...
			slicePredicate.setSlice_range(new SliceRange(
					CassandraUtils.emptyByteArray,
					CassandraUtils.finalTokenBytes, false, 100));
		} else {
		...
		try {
			Map<ByteBuffer, List<ColumnOrSuperColumn>> docMap = context.getClient()
					.multiget_slice(
							Arrays.asList(keyMap.keySet().toArray(
									new ByteBuffer[] {})), columnParent,
							slicePredicate, context.getConsistencyLevel());

			if (keyMap.size() != docMap.size()) {
				logger.warn("Missing documents in multiget_slice call");
			}

			for (Map.Entry<ByteBuffer, List<ColumnOrSuperColumn>> entry : docMap
					.entrySet()) {
				...
				if (cols == null) {
					logger.warn("Missing document in multiget_slice for: "
							+ new String(entry.getKey().array(), entry.getKey()
									.position() + entry.getKey().arrayOffset(),
									entry.getKey().remaining(), "UTF-8"));
					continue;
				}
				...
				for (ColumnOrSuperColumn col : cols) {
					...
					String fieldName = new String(name.array(), name.position()
							+ name.arrayOffset(), name.remaining());

					// Incase __META__ slips through
					if (col.column.name
							.equals(CassandraUtils.documentMetaFieldBytes)) {
					...
					if (v.array()[vlimit - 1] != Byte.MAX_VALUE
							&& v.array()[vlimit - 1] != Byte.MIN_VALUE) {
						throw new CorruptIndexException(
								"Lucandra field is not properly encoded: "
										+ docNum + "(" + fieldName + ")");

					} else if (v.array()[vlimit - 1] == Byte.MAX_VALUE) { // Binary
						value = new byte[v.remaining() - 1];
						System.arraycopy(v.array(),
								v.position() + v.arrayOffset(), value, 0,
								v.remaining() - 1);
						...
						if (fieldString.indexOf(CassandraUtils.delimeter) >= 0) {
							StringTokenizer tok = new StringTokenizer(
									fieldString, CassandraUtils.delimeter);
							while (tok.hasMoreTokens()) {
								field = new Field(fieldName, tok.nextToken(),
										Store.YES, Index.ANALYZED);
								cacheDoc.add(field);
							}
						} else {

							field = new Field(fieldName, fieldString,
									Store.YES, Index.ANALYZED);
							cacheDoc.add(field);
						}
					}
	...
	public TermFreqVector getTermFreqVector(int docNum, String field)
		...
		TermFreqVector termVector = new lucandra.TermFreqVector(indexName,
				field, docId, context);
		...
		if (logger.isDebugEnabled()) {
			try {
				logger.debug("adding docId "
						+ new String(id.array(), id.position()
								+ id.arrayOffset(), id.remaining(), "UTF-8"));
			} catch (UnsupportedEncodingException e) {
				throw new RuntimeException(e);
			}
		}
		...
		if (idx == null) {
			...
			for (Column c : docInfo.columns) {
				if (c.name.equals(CassandraUtils.normsKeyBytes)) {
					if (c.value.remaining() != 1)
						throw new IllegalStateException("Norm for field "
								+ field + " must be a single byte");

					norm = c.value.array()[c.value.position()
							+ c.value.arrayOffset()];
				}
			}
			...
			if (norms == null) {
				...
				if ((idx + 1) >= norms.length) {
					...
					byte[] _norms = new byte[(norms.length * 2) < numDocs ? (norms.length * 2)
							: (numDocs + 1)];
